"""
utils.py contains some functions that can be used for batchproccessing .tak files using scripts.
Has to be run using Ironpython 2.7
Copied from: C:/Program Files/OptiTrack/Motive/Samples/NMotive/IronPython/utils.py
Simon: Only done some minor changes
2019-07-30
"""
...
def changeFileExtension(fileName, addition, newExtension):
    ...
    return '{0}{1}{2}{3}'.format(nameWithoutExtenion, addition, dotString, newExtension)
...
def processAndExportTakes(dir, processors, exporter):
    ...
    for t in takesInDirectory(dir):
        ...
        if (not exportResult.Success):
            ...
            logger.info('Take %s successfully exported to %s format, file %s', t.Name, exporter.Extension,
                        exportedFilePath)
    return
